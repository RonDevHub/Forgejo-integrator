...
export default class ForgejoPlugin extends Plugin {
  ...
  private parseUrl(rawUrl: string): { owner: string; repo: string; index: string } | null {
    try {
      const url = new URL(rawUrl);
      const parts = url.pathname.split('/').filter(p => p.length > 0);
      
      // Erwartetes Format: /owner/repo/pulls/123 oder /owner/repo/issues/123
      if (parts.length >= 4) {
        return {
          owner: parts[0],
          repo: parts[1],
  ...
  private async renderForgejoItem(rawUrl: string, el: HTMLElement, type: 'pr' | 'issue') {
    ...
    try {
      ...
      container.empty();
      container.createEl('p', { text: `Fehler beim Abrufen: ${err.message}`, cls: 'mod-warning' });
    }
  }

  private buildTable(parent: HTMLElement, title: string, rows: [string, string][]) {
...
class ForgejoSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc('Die Basis-URL deiner Forgejo-Instanz (z. B. https://forgejo.de domain)')
      .addText(text => text
        .setPlaceholder('https://forgejo.de')
