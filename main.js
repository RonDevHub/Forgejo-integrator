...
var TRANSLATIONS = {
  ...
  de: {
    ...
    issues: "Issues",
    repository: "Repository",
    userReposOverview: "Benutzer-Repositories \xDCbersicht",
    configWarning: "\u26A0\uFE0F Bitte Server-URL und API-Token in den Einstellungen konfigurieren.",
    invalidUrl: "\u274C Ung\xFCltige URL oder nicht autorisierte Domain",
    invalidUsername: "\u274C Ung\xFCltige Eingabe angegeben.",
    fetchError: "Fehler beim Abrufen",
    contributions: "Beitr\xE4ge in den letzten",
    less: "Weniger",
    more: "Mehr"
  }
};
...
var ForgejoPlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.registerMarkdownCodeBlockProcessor(
      "FPR",
      (source, el) => this.renderForgejoSingleItem(source.trim(), el, "pr")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FIS",
      (source, el) => this.renderForgejoSingleItem(source.trim(), el, "issue")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FIS-ALL",
      (source, el) => this.renderForgejoList(source.trim(), el, "issue", "all")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FIS-OPEN",
      (source, el) => this.renderForgejoList(source.trim(), el, "issue", "open")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FIS-CLOSED",
      (source, el) => this.renderForgejoList(source.trim(), el, "issue", "closed")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FPR-ALL",
      (source, el) => this.renderForgejoList(source.trim(), el, "pr", "all")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FPR-OPEN",
      (source, el) => this.renderForgejoList(source.trim(), el, "pr", "open")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FPR-CLOSED",
      (source, el) => this.renderForgejoList(source.trim(), el, "pr", "closed")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FPR-LIST",
      (source, el) => this.renderUserPRList(source.trim(), el, "all")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FPR-LIST-OPEN",
      (source, el) => this.renderUserPRList(source.trim(), el, "open")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FPR-LIST-CLOSED",
      (source, el) => this.renderUserPRList(source.trim(), el, "closed")
    );
    this.registerMarkdownCodeBlockProcessor(
      "FRI",
      (source, el) => this.renderForgejoList(source.trim(), el, "issue", "all")
    ...
    this.registerMarkdownCodeBlockProcessor(
      "FR",
      (source, el) => this.renderRepoDetails(source.trim(), el)
    );
    this.registerMarkdownCodeBlockProcessor(
      "FR-ALL",
      (source, el) => this.renderAllUserRepos(el)
    );
    this.registerMarkdownCodeBlockProcessor(
      "FRA",
      (source, el) => this.renderActivityHeatmap(source.trim(), el, 6)
    );
    for (let m = 1; m <= 12; m++) {
      this.registerMarkdownCodeBlockProcessor(
        `FRA-${m}`,
        (source, el) => this.renderActivityHeatmap(source.trim(), el, m)
      );
    }
  ...
  async fetchRepoHeatmapFromCommits(owner, repo) {
    const commitCounts = {};
    let page = 1;
    const limit = 50;
    ...
    while (hasMorePages) {
      ...
      const commits = await this.fetchApi(endpoint);
    ...
    return Object.entries(commitCounts).map(([dateStr, count]) => {
      const timestampInSeconds = Math.floor(new Date(dateStr).getTime() / 1e3);
      return {
  ...
  async fetchRawFile(owner, repo, filepath) {
    try {
      const baseUrl = this.settings.serverUrl.replace(/\/$/, "");
      const targetUrl = new URL(
  ...
  async detectLicense(owner, repo) {
    const files = [
      "LICENSE",
      "LICENSE.md",
      "LICENSE.txt",
    ...
    for (const file of files) {
      ...
      if (content) {
        ...
        if (text.includes("GNU GENERAL PUBLIC LICENSE") || text.includes("GPL")) {
          if (text.includes("VERSION 3"))
            return "GPL-3.0";
          if (text.includes("VERSION 2"))
            return "GPL-2.0";
          return "GPL";
        }
        if (text.includes("GNU AFFERO GENERAL PUBLIC LICENSE") || text.includes("AGPL"))
          return "AGPL-3.0";
  ...
  formatUser(user) {
    ...
    const avatar = cleanAvatar && cleanAvatar !== "#" ? `<img src="${cleanAvatar}" class="forgejo-avatar" alt="${cleanLogin}" />` : "";
  ...
  formatDate(dateStr) {
    ...
    return isNaN(d.getTime()) ? "N/A" : escapeHtml(d.toLocaleDateString(locale));
  }
  extractPushDate(repo) {
    const rawDate = repo.pushed_at || repo.updated_at || repo.updated;
    return this.formatDate(rawDate);
  }
  renderVisibility(isPrivate) {
    const icon = isPrivate ? SVG_LOCK : SVG_GLOBE;
    const text = isPrivate ? this.t("private") : this.t("public");
    return `<span class="forgejo-inline-icon">${icon} <span>${text}</span></span>`;
  }
  async renderForgejoSingleItem(rawUrl, el, type) {
    el.empty();
    ...
    if (!parsed || !parsed.index) {
      container.createEl("p", {
        text: `${this.t("invalidUrl")}: "${escapeHtml(rawUrl)}"`,
        cls: "mod-warning"
      });
      return;
    }
    ...
    try {
      ...
      if (type === "pr") {
        ...
        this.buildSingleTable(container, `Pull Request #${data.number}`, [
          [this.t("title"), escapeHtml(data.title || "N/A")],
          [this.t("status"), this.formatStatus(data.state)],
          [this.t("author"), this.formatUser(data.user)],
          [this.t("created"), this.formatDate(data.created_at)],
          [
            this.t("link"),
            `<a href="${sanitizeUrl(data.html_url, this.settings.serverUrl)}" target="_blank" rel="noopener">${this.t("open")}</a>`
          ]
        ]);
      } else {
        const data = await this.fetchApi(endpoint);
        container.empty();
        const labels = data.labels && data.labels.length > 0 ? data.labels.map((l) => escapeHtml(l.name)).join(", ") : this.t("none");
        this.buildSingleTable(container, `Issue #${data.number}`, [
          [this.t("title"), escapeHtml(data.title || "N/A")],
          [this.t("status"), this.formatStatus(data.state)],
          [this.t("author"), this.formatUser(data.user)],
          [this.t("labels"), labels],
          [this.t("created"), this.formatDate(data.created_at)],
          [
            this.t("link"),
            `<a href="${sanitizeUrl(data.html_url, this.settings.serverUrl)}" target="_blank" rel="noopener">${this.t("open")}</a>`
  ...
  async renderForgejoList(rawUrl, el, type, state) {
    el.empty();
    const container = el.createDiv({ cls: "forgejo-container" });
    if (!this.settings.apiToken || !this.settings.serverUrl) {
      container.createEl("p", {
    ...
    container.createEl("span", { text: this.t("loading") });
    try {
      const endpoint = type === "pr" ? `repos/${parsed.owner}/${parsed.repo}/pulls?state=${state}` : `repos/${parsed.owner}/${parsed.repo}/issues?state=${state}`;
      const items = await this.fetchApi(endpoint);
      container.empty();
      const typeName = type === "pr" ? this.t("pullRequests") : this.t("issues");
      const title = `${typeName} (${state.toUpperCase()}) - ${escapeHtml(parsed.owner)}/${escapeHtml(parsed.repo)}`;
      const headers = [
        this.t("id"),
        this.t("title"),
        this.t("status"),
      ...
      const rows = items.map((item) => [
        ...
        `<a href="${sanitizeUrl(item.html_url, this.settings.serverUrl)}" target="_blank" rel="noopener">${this.t("open")}</a>`
      ]);
  ...
  async renderUserPRList(username, el, state) {
    el.empty();
    const container = el.createDiv({ cls: "forgejo-container" });
    if (!this.settings.apiToken || !this.settings.serverUrl) {
      container.createEl("p", {
    ...
    container.createEl("span", {
      text: `${this.t("loading")} (${escapeHtml(cleanUser)})...`
    });
    try {
      ...
      container.empty();
      const title = `${this.t("pullRequests")} (${state.toUpperCase()}) - ${this.t("author")}: ${escapeHtml(cleanUser)}`;
      const headers = [
        this.t("repo"),
        this.t("id"),
        this.t("title"),
  ...
  async renderRepoDetails(rawUrl, el) {
    el.empty();
    const container = el.createDiv({ cls: "forgejo-container" });
    const parsed = this.parseUrl(rawUrl);
    if (!parsed) {
    ...
    container.createEl("span", { text: this.t("loading") });
    try {
      const repo = await this.fetchApi(
      ...
      let lastReleaseTag = this.t("none");
      try {
      ...
      this.buildStructuredTable(
        container,
        `${this.t("repository")}: ${escapeHtml(repo.full_name)}`,
        headers,
        rows
      );
    } catch (err) {
  ...
  async renderAllUserRepos(el) {
    el.empty();
    const container = el.createDiv({ cls: "forgejo-container" });
    container.createEl("span", { text: this.t("loading") });
    try {
      const repos = await this.fetchApi("user/repos");
      container.empty();
      const headers = [
        this.t("name"),
        this.t("visibility"),
      ...
      const rowsData = await Promise.all(
        repos.map(async (repo) => {
          ...
          const lastRelease = releases && releases.length > 0 ? escapeHtml(releases[0].tag_name || releases[0].name) : this.t("none");
          return [
  ...
  async renderActivityHeatmap(input, el, monthsCount) {
    el.empty();
    const container = el.createDiv({ cls: "forgejo-container" });
    if (!this.settings.apiToken || !this.settings.serverUrl) {
      container.createEl("p", {
    ...
    try {
      ...
      if (parsed) {
        ...
        heatmapData = await this.fetchApi(endpoint);
      }
      container.empty();
      ...
      if (Array.isArray(heatmapData)) {
        heatmapData.forEach((item) => {
          const d = new Date(item.timestamp * 1e3);
      ...
      const duotoneSvg = (color) => `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="11" height="11" class="forgejo-duotone-icon" style="color: ${color};"><path opacity=".4" fill="currentColor" d="M32 96l0 320c0 17.7 14.3 32 32 32l320 0c17.7 0 32-14.3 32-32l0-320c0-17.7-14.3-32-32-32L64 64C46.3 64 32 78.3 32 96z"/><path fill="currentColor" d="M384 64c17.7 0 32 14.3 32 32l0 320c0 17.7-14.3 32-32 32L64 448c-17.7 0-32-14.3-32-32L32 96c0-17.7 14.3-32 32-32l320 0zM64 32C28.7 32 0 60.7 0 96L0 416c0 35.3 28.7 64 64 64l320 0c35.3 0 64-28.7 64-64l0-320c0-35.3-28.7-64-64-64L64 32z"/></svg>`;
      const boxSize = 11;
      const boxGap = 3;
      const leftOffset = 28;
      const topOffset = 20;
      ...
      const monthNames = this.settings.language === "de" ? monthNamesDe : monthNamesEn;
      let lastMonth = -1;
      ...
      const dayLabels = this.settings.language === "de" ? ["Mo", "", "Mi", "", "Fr", "", ""] : ["Mon", "", "Wed", "", "Fri", "", ""];
  ...
  buildStructuredTable(parent, title, headers, rows, fallbackText) {
    ...
    const renderTbody = () => {
      ...
      if (currentRows.length === 0) {
        ...
        emptyTd.textContent = fallbackText || this.t("noData");
        return;
      }
    ...
    const updateHeaders = () => {
      ...
      headers.forEach((h, index) => {
        ...
        if (h.toLowerCase() === this.t("link").toLowerCase()) {
          th.textContent = h;
          return;
        }
        if (rows.length > 1) {
          ...
          if (sortColumnIndex === index) {
            ...
            iconContainer.firstChild?.setAttribute(
              "style",
              "opacity: 0.2;"
            );
          }
          th.addEventListener("click", () => {
            ...
            currentRows.sort((a, b) => {
              ...
              return sortAscending ? valA.localeCompare(valB, void 0, {
                numeric: true,
                sensitivity: "base"
              }) : valB.localeCompare(valA, void 0, {
                numeric: true,
                sensitivity: "base"
              });
            });
  ...
  buildSingleTable(parent, typeTitle, rows) {
    const table = parent.createEl("table", { cls: "forgejo-table" });
    const thead = table.createEl("thead");
    if (this.settings.tableLayout === "vertical") {
      ...
      for (const [key, val] of rows) {
        ...
        if (val.includes("<a ") || val.includes("<div ") || val.includes("<span ") || val.includes("<img ") || val.includes("<svg "))
          tdVal.innerHTML = val;
        else
...
var ForgejoSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    const infoBox = containerEl.createDiv({
      ...
      attr: {
        style: "margin-bottom: 20px; padding: 12px; background-color: var(--background-secondary); border-radius: 6px;"
      }
    });
    infoBox.createEl("h3", {
    ...
    new import_obsidian.Setting(containerEl).setName("Forgejo Server URL").setDesc(
      ...
      (text) => text.setPlaceholder("https://my-forgejo-instance.com").setValue(this.plugin.settings.serverUrl).onChange(async (value) => {
        this.plugin.settings.serverUrl = value.trim();
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("API Token").setDesc("Personal Access Token (Read permissions required)").addText((text) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Display Language").setDesc("Select the output language for rendered tables.").addDropdown(
      (dropdown) => dropdown.addOption("en", "English (Default)").addOption("de", "Deutsch").setValue(this.plugin.settings.language || "en").onChange(async (value) => {
        this.plugin.settings.language = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Enable Caching").setDesc(
    ...
    new import_obsidian.Setting(containerEl).setName("Table Layout").setDesc("Choose layout style for single item views (FIS / FPR)").addDropdown(
      (dropdown) => dropdown.addOption("vertical", "Vertical (Key / Value rows)").addOption("horizontal", "Horizontal (Columns side-by-side)").setValue(this.plugin.settings.tableLayout).onChange(async (value) => {
        this.plugin.settings.tableLayout = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Table Theme").setDesc("Select color theme for rendered tables").addDropdown(
      (dropdown) => dropdown.addOption("dark", "Dark Contrast").addOption("light", "Light Clean").addOption("blue", "Modern Blue").addOption("purple", "Purple Accent").addOption("mono", "Mono").addOption("sepia", "Sepia").addOption("nord", "Nord").addOption("dracula", "Dracula").addOption("cyberpunk", "Cyberpunk").addOption("midnight", "Midnight").addOption("slate", "Slate").addOption("teal", "Teal").addOption("amber", "Amber").addOption("coffee", "Coffee").addOption("synthwave", "Synthwave").addOption("solarized-dark", "Solarized Dark").addOption("rose", "Rose").addOption("emerald", "Emerald").setValue(this.plugin.settings.themeStyle).onChange(async (value) => {
        this.plugin.settings.themeStyle = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Refresh Interval").setDesc(
      "Select how often data tables automatically update in the background."
    ).addDropdown(
      (dropdown) => dropdown.addOption("0", "Realtime (On view / render)").addOption("60000", "1 Minute (Default)").addOption("300000", "5 Minutes").addOption("1800000", "30 Minutes").addOption("3600000", "1 Hour").setValue(String(this.plugin.settings.refreshInterval)).onChange(async (value) => {
        this.plugin.settings.refreshInterval = Number(value);
        await this.plugin.saveSettings();
      })
    );
    const testSetting = new import_obsidian.Setting(containerEl).setName("Test Connection").setDesc("Verify instance URL reachability and API Token validity.");
    ...
    testSetting.addButton(
      (button) => button.setButtonText("Test Connection").setCta().onClick(async () => {
        ...
        statusContainer.createEl("span", {
          text: "\u{1F504} Testing connection..."
        });
        try {
          const user = await this.plugin.fetchApi("user");
          statusContainer.empty();
          ...
          new import_obsidian.Notice("Forgejo connection successful!");
        } catch (err) {
          statusContainer.empty();
          statusContainer.createEl("div", {
          ...
          new import_obsidian.Notice("Forgejo connection failed.");
        }
      })
    );
    ...
    const badgeContainer = supportContainer.createDiv({
      attr: {
        style: "display: flex; gap: 12px; justify-content: center; align-items: center; flex-wrap: wrap;"
      }
    });
